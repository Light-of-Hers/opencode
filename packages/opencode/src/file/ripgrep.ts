...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const resolveFollow = Effect.fnUntraced(function* (follow: boolean | undefined) {
      if (follow !== undefined) return follow
      return yield* config.get().pipe(
        Effect.map((current) => current.follow_symlinks ?? true),
        Effect.catch(() => Effect.succeed(true)),
      )
    })
    const resolveFilesInput = Effect.fnUntraced(function* (input: FilesInput) {
      return {
        ...input,
        follow: yield* resolveFollow(input.follow),
      } satisfies FilesInput
    })
    const resolveSearchInput = Effect.fnUntraced(function* (input: SearchInput) {
      return {
        ...input,
        follow: yield* resolveFollow(input.follow),
    ...
    const source = (input: FilesInput) => {
      return Stream.unwrap(
        Effect.gen(function* () {
          const resolved = yield* resolveFilesInput(input)
          const useWorker = !!resolved.signal && typeof Worker !== "undefined"
          if (!useWorker && resolved.signal) {
            log.warn("worker unavailable, ripgrep abort disabled")
          }
          return useWorker ? filesWorker(resolved) : filesDirect(resolved)
        }),
      )
    }
    ...
    const search: Interface["search"] = Effect.fn("Ripgrep.search")(function* (input: SearchInput) {
      const resolved = yield* resolveSearchInput(input)
      const useWorker = !!resolved.signal && typeof Worker !== "undefined"
      if (!useWorker && resolved.signal) {
        log.warn("worker unavailable, ripgrep abort disabled")
      }
      return yield* useWorker ? searchWorker(resolved) : searchDirect(resolved)
    })
